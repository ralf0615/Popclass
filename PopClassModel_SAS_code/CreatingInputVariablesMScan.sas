...
%Let MSversion    =142;
%Let MSversionNext=151;
%Let StartDate='31MAR2014'd; /* always choose the end of month */
%let version=31MAR2014MarketScan10PercentSample;
%Let AEoutpath=/rpscan/u0086308/POPSTRATIFICATION/AEout/&version.; 
%Let outpath=/rpscan/u0086308/POPSTRATIFICATION/output/&version.;
%Let username=gsirbu;

x "mkdir -p /rpscan/u0086308/POPSTRATIFICATION/temp/&version.";
x "mkdir -p /rpscan/u0086308/POPSTRATIFICATION/AEout/&version.";
x "mkdir -p /rpscan/u0086308/POPSTRATIFICATION/output/&version.";
libname temp "/rpscan/u0086308/POPSTRATIFICATION/temp/&version.";

/*** create the sample file if needed ***/

/* sample for client 450 for Q1 *
%Let lastmonth=%sysfunc(month(&StartDate.),2.);
proc sql; 
create table temp.SamplewClassification as select
	enrolid,
	' ' as AssignmentFinal
from arch&year..ccaea&MSversion.
where client=450 and enrind&lastmonth.=1; *must be enrolled in the last month *;
quit;
*/

%macro setup;
%if &StartDate. ne 31DEC&year. %then %do;
...
* create a clone MarketScan ENROLLMENT FILE data based on the reference date and looking back 1 year, same for the forward year *;
	...
	merge 		
		arch&year..ccaea&MSversion.(keep=enrolid cluster age sex msa rx enrind1-enrind%sysfunc(month(&StartDate.)) plntyp1-plntyp%sysfunc(month(&StartDate.)) in=a) 
	...
	merge 		
		arch&year..ccaea&MSversion.(keep=enrolid cluster age sex msa rx enrind%eval(%sysfunc(month(&StartDate.))+1)-enrind12 plntyp%eval(%sysfunc(month(&StartDate.))+1)-plntyp12 in=a) 
...
%macro libsetup;
%if &StartDate. ne 31DEC&year. %then %do;
	libname arch&year. "/rpscan/u0086308/POPSTRATIFICATION/temp/&version.";
	libname arch&Nextyear. "/rpscan/u0086308/POPSTRATIFICATION/temp/&version.";
%end;
%mend libsetup;
%libsetup;


/* 10% SRS without replacement from MarketScan *
proc surveyselect
	data=arch&year..ccaea&MSversion.(where=(enrind12=1))
	rate=0.1
	method=srs
	out=temp.SamplewClassification;
id enrolid;
run;
data temp.SamplewClassification; 
set temp.SamplewClassification; 
length AssignmentFinal $ 1; 
run;
*/

/* export the sample to a csv file if needed *
proc export data=trash
	outfile="/rpscan/u0086308/POPSTRATIFICATION/data/AllClientSample&version..csv" 
	replace;
run;
*/
...
*** run AE for DS using above configuration file;
/*x "&AEpath./AnalyticsEngine.sh &AEpath./Configfile.properties"; AE not working, please keep investigating*/


rsubmit;
/* import AE back into the data */
data temp.SampleIPOPwDS1;
	infile "&AEoutpath./&username._DISEASESTAGING6.33_&version._Dsoutput.csv"
	delimiter = ',' 
	MISSOVER 
	DSD 
	lrecl=32767
