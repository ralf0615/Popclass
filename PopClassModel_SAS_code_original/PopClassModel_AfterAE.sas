...
/*** create financial variables from past year for testing and analysis of the classification ***/
data trash (keep=month name1-name5);
	retain month year;
		if month(&startdate.)=12 then do; 
			month=1;
			year=year(&startdate.); 
		end;
		else do;
			month=month(&startdate.)+1;
			year=year(&startdate.)-1;
		end;
	do i=1 to 12;
		name1=put(mdy(month,1,year),monyy7.)||'PayInp';
		name2=put(mdy(month,1,year),monyy7.)||'PayRx';
		name3=put(mdy(month,1,year),monyy7.)||'PayOtp';
		name4='enrind'||left(put(month,2.));
		name5=put(mdy(month,1,year),monyy7.)||'Enr';
		output;
		if month<12 then month=month+1;
		else do; month=1;year=year+1; end;
	end;
run;
proc sql noprint;
select 
	...
	name5 
into 
	:monthnumber1-:monthnumber12, 
	:monthlistPayInp separated by ' ', 
	:monthlistPayRx  separated by ' ', 
	:monthlistPayOtp separated by ' ', 
	:enrlist         separated by ' ',
	:monthlistEnr    separated by ' ' 
from trash;

%macro monthlistpast;
* Inpatient past year*;
proc sql;
create table trash801 as select
	b.enrolid,
	month(a.admdate) as month,
	sum(a.totpay) as pay
from 
	temp.SamplewClassification	as b
left join
...
group by
	...
	calculated month;
quit;
data trash802(keep=enrolid &monthlistPayInp.);									/*** <<<<--- merge for testing analyssi ***/
set trash801;
by enrolid;
array m[12] &monthlistPayInp.;
retain &monthlistPayInp.;
if first.enrolid then do i=1 to 12; m[i]=0; end;
%do i=1 %to 12;
	if month=&&monthnumber&i. then m[&i]=pay;
%end;
...
* Drugs past year*;
proc sql;
create table trash803 as select
	b.enrolid,
	month(a.svcdate) as month,
	sum(a.pay) as pay
from 
	temp.SamplewClassification	as b
left join
...
group by
	...
	calculated month;
quit;
data trash804(keep=enrolid &monthlistPayRx.);									/*** <<<<--- merge for testing analyssi ***/
set trash803;
by enrolid;
array m[12] &monthlistPayRx.;
retain &monthlistPayRx.;
if first.enrolid then do i=1 to 12; m[i]=0; end;
%do i=1 %to 12;
	if month=&&monthnumber&i. then m[&i]=pay;
%end;
...
* Outpatient past year*;
proc sql;
create table trash805 as select
	b.enrolid,
	month(a.svcdate) as month,
	sum(a.pay) as pay
from 
...
group by
	...
	calculated month;
quit;
data trash806(keep=enrolid &monthlistPayOtp.);									/*** <<<<--- merge for testing analyssi ***/
set trash805;
by enrolid;
array m[12] &monthlistPayOtp.;
retain &monthlistPayOtp.;
if first.enrolid then do i=1 to 12; m[i]=0; end;
%do i=1 %to 12;
	if month=&&monthnumber&i. then m[&i]=pay;
%end;
...
*** get the demographics and enrollment data from past year;
PROC SQL;
create table trash813 as select 											/*** <<<<--- merge for testing analyssi ***/
	b.enrolid,
	a.age,
	a.sex,
	a.msa,
	a.rx,
	%do i=1 %to 12;
    %scan(&enrlist.,&i.) as %scan(&monthlistEnr.,&i.),
	%end;
	coalesce(a.plntyp12,a.plntyp11,a.plntyp10,a.plntyp9,a.plntyp8,a.plntyp7,a.plntyp6,a.plntyp5,a.plntyp4,a.plntyp3,a.plntyp2,a.plntyp1) as plantyp&year.
from 
	temp.SamplewClassification	as b
left join
	arch&year..ccaea&MSversion.			as a
on
	a.enrolid=b.enrolid
order by
	b.enrolid;
...
/*** create financial variables from next year for testing and analysis of the classification ***/
data trash (keep=month name1-name5);
	retain month year;
		if month(&startdate.)=12 then do; 
			month=1;
...
* Inpatient next year*;
proc sql;
create table trash807 as select
	b.enrolid,
	month(a.admdate) as month,
	sum(a.totpay) as pay
from 
	temp.SamplewClassification	as b
left join
	arch&Nextyear..ccaei&MSversionNext.		as a
on
	a.enrolid=b.enrolid
group by
	b.enrolid,
	calculated month;
quit;
data trash808(keep=enrolid &monthlistPayInp.);							/*** <<<<--- merge for testing analyssi ***/
set trash807;
by enrolid;
array m[12] &monthlistPayInp.;
retain &monthlistPayInp.;
...
* Drugs next year*;
proc sql;
create table trash809 as select
	b.enrolid,
	month(a.svcdate) as month,
	sum(a.pay) as pay
from 
	temp.SamplewClassification	as b
left join
	arch&Nextyear..ccaed&MSversionNext.			as a
on
	a.enrolid=b.enrolid
group by
	b.enrolid,
	calculated month;
quit;
data trash810(keep=enrolid &monthlistPayRx.);							/*** <<<<--- merge for testing analyssi ***/
set trash809;
by enrolid;
array m[12] &monthlistPayRx.;
retain &monthlistPayRx.;
...
* Outpatient next year*;
proc sql;
create table trash811 as select
	b.enrolid,
	month(a.svcdate) as month,
	sum(a.pay) as pay
from 
	temp.SamplewClassification	as b
left join
	arch&Nextyear..ccaeo&MSversionNext.			as a
on
	a.enrolid=b.enrolid
group by
	b.enrolid,
	calculated month;
quit;
data trash812(keep=enrolid &monthlistPayOtp.);							/*** <<<<--- merge for testing analyssi ***/
set trash811;
by enrolid;
array m[12] &monthlistPayOtp.;
retain &monthlistPayOtp.;
...
*** get the demographics and enrollment data from next year;
PROC SQL;
create table trash814 as select 												/*** <<<<--- merge for testing analyssi ***/
	b.enrolid,
	%do i=1 %to 12;
    %scan(&enrlist.,&i.) as %scan(&monthlistEnr.,&i.),
	%end;
	coalesce(a.plntyp12,a.plntyp11,a.plntyp10,a.plntyp9,a.plntyp8,a.plntyp7,a.plntyp6,a.plntyp5,a.plntyp4,a.plntyp3,a.plntyp2,a.plntyp1) as plantyp&Nextyear.
from 
	temp.SamplewClassification	as b
left join
	arch&Nextyear..ccaea&MSversionNext.			as a
on
	a.enrolid=b.enrolid
order by
	b.enrolid;
...
*** get the deceased information from next year inpatient discharges;
proc sql;
create table trash815 as select														/*** <<<<--- merge for testing analyssi ***/
	b.enrolid,
	max(case a.dstatus when '20' then 1 else 0 end) as flag_death,
	max(case a.dstatus when '20' then month(a.admdate) else . end) as month_death,
	max(case a.dstatus when '20' then a.drg else . end) as drg_death
from 
	temp.SamplewClassification	as b
left join
	arch&Nextyear..ccaei&MSversionNext.			as a
...
*** other utilization variables from the past year;
*** all admissions from last year;
proc sql;
create table trash816 as															/*** <<<<--- merge for testing analyssi ***/
select
	b.enrolid,
	count(distinct a.admdate) as CountAdm12mAll
from 
	temp.SamplewClassification as b
left join
	arch&year..ccaei&MSversion.(where=(&StartDate. >= admdate > &StartDate.-366)) as a
on
	a.enrolid=b.enrolid
group by
	b.enrolid;
...
*** all admissions from next year;
proc sql;
create table trash817 as																/*** <<<<--- merge for testing analyssi ***/
select
	b.enrolid,
	count(distinct a.admdate) as CountAdm12mNextYearAll
from 
	temp.SamplewClassification	as b
left join
	arch&Nextyear..ccaei&MSversionNext.(where=(&StartDate. < admdate <= &StartDate.+366)) as a
on
	a.enrolid=b.enrolid
group by
	b.enrolid;
...
*** all admissions from next 3 months;
proc sql;
create table trash818 as																	/*** <<<<--- merge for testing analyssi ***/
select
	b.enrolid,
	count(distinct a.admdate) as CountAdm3mNextYearAll
from 
	temp.SamplewClassification	as b
left join
	arch&Nextyear..ccaei&MSversionNext.(where=(&StartDate. < admdate <= &StartDate.+90)) as a
on
	a.enrolid=b.enrolid
group by
	b.enrolid;
...
*** count ER visits for the next 3 months;
proc sql;
create table trash819 as 
select
	c.*,
	d.caseid 
from
	(
		select 
			...
			b.fachdid
		from
			temp.SamplewClassification as a,
			arch&Nextyear..ccaeo&MSversionNext. as b
		where
			a.enrolid=b.enrolid and
			&StartDate. < svcdate <= &StartDate. + 90 			/* next 3 months */
	) as c
left join 
	arch&Nextyear..ccaef&MSversionNext. as d
on
	c.enrolid=d.enrolid and
	c.fachdid=d.fachdid;

create table trash820 as																/*** <<<<--- merge for testing analyssi ***/
select
	b.enrolid,
	count(distinct a.msclmid) as Count3mNextYearER
from 
	temp.SamplewClassification	as b
left join
	trash819(where=
		(
			&StartDate. < svcdate <= &StartDate. + 90	and		/*next 3 months*/
			facprof='F'									and		/*Facility claim*/
			substr(svcscat,4,2)="20"					and		/*ER visits*/
			missing(caseid)=1							and 	/*no inpatient*/
			^(proc1 in (&ProcExclusionList.))					/*exclusion based on procedures*/
...
*** highest stage in the past year ***
*** transpose from AE output based on both IP and OP the Dxcat + Stage to have one per record, also apply the exclusion criteria;
data trash821(keep=enrolid svcdate rec_type svcscat proc1 DxCat DxStage);
	set temp.SampleIPOPwDS1;
	where svcdate > &StartDate. - 366; 			/* past year */	
	array c[4] dxcat1  -dxcat4;
	array s[4] dxstage1-dxstage4;
	do i=1 to 4;
		if (rec_type='O' and (substr(svcscat,4,2) in (&SvcCatExclusionList.) or proc1 in (&ProcExclusionList.))) then do; c[i]=' '; s[i]=.; end;
		if 
			(missing(c[i])=0 and missing(s[i])=0)
		then do;
			DxCat=c[i];
			DxStage=s[i];
			output;
...
run;
proc sql;
create table trash822 as
select 
	b.enrolid,
	a.DxCat as DxCatFromMaxDxStage,
	a.DxStage as MaxDxStage
from
	temp.SamplewClassification	as b
left join 
	trash821 as a
...
group by
	b.enrolid
having
	a.DxStage=max(a.DxStage);
proc sort data=trash822 out=trash823 nodupkey; by enrolid; run;									/*** <<<<--- merge for testing analyssi ***/

/****************************/
/*** FINAL OUTPUT TESTING ***/
/****************************/
*** put together all the data for testing analysis ***;
DATA
	temp.AllRulesAndMoreTestingVariables;
merge
	temp.AllRules (in=a)
	trash802 (in=b)
	trash804 (in=c)
...
run;
proc export 
	data=temp.AllRulesAndMoreTestingVariables
	outfile="&outpath./AllRulesAndMoreTestingVariables_&version..csv" 
	replace;
run;
